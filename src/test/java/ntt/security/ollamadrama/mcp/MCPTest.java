...
public class MCPTest {
	...
	/**
	 * This test is intended to trigger an MCP call to 'fetch' tool to verify availability of the ntt.com site.  
	 * Note: requires running a fetch MCP server on port 8080
	 *  - Lists available tools and finds 'fetch'
	...
	/**
	 * Asks for the current temperature in Paris
	 * - First without tools available - LLMs expected to fail
	 * - Second with simulated tool listed as available - LLMs expected to call tool
	 * - Third with a simulated tool reply - LLMs expected to trust provided input
	 */
	@SuppressWarnings("serial")
	@Test
	public void simple_HTTP_MCP_Temperature_LLMToolCallOutput_Test() {
		...
		scorecard1.evaluate();
		scorecard1.print();
		assertTrue("Ensure we have only have positive scores", scorecard1.isCleanSheetPositive(models));

		System.out.println("Next provide simulated information about tools available for the task, LLMs expected to make tool call request");
		System.out.println("-------------------------------------------------------");
		...
		scorecard2.evaluate();
		scorecard2.print();
		assertTrue("Ensure we have only have positive scores", scorecard2.isCleanSheetPositive(models));
		...
		// Print the scorecard
		System.out.println("SCORECARD 3:");
		scorecard3.evaluate();
		scorecard3.print();
		assertTrue("Ensure we have only have positive scores", scorecard3.isCleanSheetPositive(models));
